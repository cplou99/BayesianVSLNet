...
class OurScaledVisualProjection(nn.Module):
    def __init__(self, visual_dim, dim, drop_rate=0.0):
        print('Loading OurScaledVisualProjection...----------------')
        super(OurScaledVisualProjection, self).__init__()
        self.drop = nn.Dropout(p=drop_rate)
        self.linear_1 = Conv1D(
            in_dim=visual_dim,
            out_dim=visual_dim // 4,
            kernel_size=1,
            stride=1,
            bias=True,
            padding=0,
        )
        self.linear_2 = Conv1D(
            in_dim=visual_dim // 4,
            out_dim=visual_dim // 8,
            kernel_size=1,
            stride=1,
            bias=True,
            padding=0,
        )
        self.linear_3 = Conv1D(
            in_dim=visual_dim // 8,
            out_dim=dim,
            kernel_size=1,
            stride=1,
        ...
        self.mlp = nn.Sequential(self.linear_1, nn.ReLU(), self.dropout, self.linear_2, nn.ReLU(), self.dropout, self.linear_3)

    def forward(self, visual_features):
        # the input visual feature with shape (batch_size, seq_len, visual_dim)
        visual_features = self.drop(visual_features)
        output = self.mlp(visual_features)  # (batch_size, seq_len, dim)
        return output
...
class OneHotConditionedPredictor(nn.Module):
    def __init__(self, dim, num_heads, max_pos_len, drop_rate=0.0, predictor="rnn", return_sigmoid=False):
        super(OneHotConditionedPredictor, self).__init__()
        self.predictor = predictor
        self.return_sigmoid = return_sigmoid
        if predictor == "rnn":
            self.encoder = DynamicRNN(dim=dim)
        else:
            self.encoder = FeatureEncoder(
                dim=dim,
                num_heads=num_heads,
    ...
    def forward(self, x, mask):
        if self.predictor == "rnn":
            features = self.encoder(x, mask)  # (batch_size, seq_len, dim)
        else:
            features = self.encoder(x, mask)
            features = self.layer_norm(features)
        features = self.start_block(
            torch.cat([features, x], dim=2)
        )  # (batch_size, seq_len, 1)
       
        logits = mask_logits(features.squeeze(2), mask=mask)
    ...
    @staticmethod
    def extract_index(probs, num_times=1, num_text_queries=1, topk=1):
        # We stored the indices and the probabilities (not necessary but only if we want to sort them after)
        start_indices_bs, end_indices_bs = [], []
        batch_size = len(probs)
        seq_lens = [len(prob) for prob in probs]

        # We get the indices of the probabilities sorted in descending order
        idxs_probs_sorted = [np.argsort(np.array(prob))[::-1] for prob in probs] # Shape (batch_size, seq_len)

        # We take as threshold the third quantile 
        # percentiles = [85 for bs in range(batch_size)]
        percentiles = [70+20*min(1, n_t_q/50) for n_t_q in num_text_queries] # 
        threshold = [np.percentile(prob, p) for prob, p in zip(probs, percentiles)]  # Shape (batch_size, )

        # Get topk predictions knowing that it appears num_times in the video (We assume that the segments are disjoint)
        for bs in range(batch_size):
            n_times = num_times[bs]
            seq_len = seq_lens[bs]
            preds_idxs, preds_probs = [], []
            for nk in range(n_times*topk):
                pred_idxs, pred_probs = [], []
                idx = idxs_probs_sorted[bs][0]
                t = threshold[bs]
                pred_idxs.append(idx)
                prob = probs[bs][idx]
                pred_probs.append(prob)
                curr_idx = idx
                while curr_idx>0 and probs[bs][curr_idx-1]>t:
                    curr_idx -= 1
                    pred_idxs.append(curr_idx)
                    prob = probs[bs][curr_idx]
                    pred_probs.append(prob)
                curr_idx = idx
                while curr_idx<seq_len-1 and probs[bs][curr_idx+1]>t:
                    curr_idx += 1
                    pred_idxs.append(curr_idx)
                    prob = probs[bs][curr_idx]
                    pred_probs.append(prob)
                
                args_pred_idxs = torch.argsort(torch.tensor(pred_idxs)).tolist()
                pred_idxs = sorted(pred_idxs)
                pred_probs = [pred_probs[i] for i in args_pred_idxs]
                preds_idxs.append(pred_idxs)
                preds_probs.append(pred_probs)
                # We remove the indices that we have already predicted
                idxs_probs_sorted[bs] = [ix for ix in idxs_probs_sorted[bs] if ix not in pred_idxs]
               
            
            start_indices, end_indices = [[] for n in range(n_times)], [[] for n in range(n_times)]
            # We sort the predictions according to init_time
            for k in range(topk):
                preds_idxs_k = preds_idxs[n_times*k:n_times*(k+1)]
                sorted_preds_idxs_k = sorted(preds_idxs_k, key=lambda x: x[0])
                start_indices_k = [idxs[0] for idxs in sorted_preds_idxs_k]
                end_indices_k = [idxs[-1] for idxs in sorted_preds_idxs_k]
                for n in range(n_times):
                    start_indices[n].append(start_indices_k[n])
                    end_indices[n].append(end_indices_k[n])
            
        
            start_indices_bs.append(start_indices)
            end_indices_bs.append(end_indices)
        
        return start_indices_bs, end_indices_bs

    @staticmethod
    def extract_index_prior(probs, num_times=1, num_text_queries=1, topk=1):
        # We stored the indices and the probabilities (not necessary but only if we want to sort them after)
        start_indices_bs, end_indices_bs = [], []
        batch_size = len(probs)
        seq_lens = [len(prob) for prob in probs]
        ...
        # We take as threshold a quantile 
        percentiles = [95 for bs in range(batch_size)]
        
        # percentiles = [75 for bs in range(batch_size)]
        threshold = [np.percentile(prob, p) for prob, p in zip(probs, percentiles)]  # Shape (batch_size, )
    ...
    @staticmethod
    def compute_binary_cross_entropy_loss(probs, labels):
        probs = torch.sigmoid(probs)
        loss = nn.BCELoss(reduction="sum")(probs, labels)
        return loss
    ...
    def sigmoid_focal_loss(
        ...
        Args:
            ...
            reduction: 'none' | 'mean' | 'sum'
                    ...
                    print(torch.unique(reg_targets, return_counts=True))
