...
from utils.data_util import (
    ...
    save_pickle,
    time_to_index,
    time_to_one_hot_index,
    batched_time_to_index
)
...
class EpisodicNLQProcessor:
    ...
    def process_data_tan(self, data, scope):
        ...
        for vid, data_item in tqdm(
            ...
            data_item['refined_sentences'] = [sentence.strip().lower().replace('.', '') for sentence in data_item['sentences']]
            zipper = zip(
                data_item["timestamps"],
                data_item["exact_times"],
                data_item["sentences"],
                data_item["refined_sentences"],
                data_item["annotation_uids"],
                data_item["query_idx"],
            )
            
            sentence_query_map = {}
            for sentence, query_idx in zip(data_item["refined_sentences"], data_item["query_idx"]):
                if sentence not in sentence_query_map:
                    sentence_query_map[sentence] = []
                sentence_query_map[sentence].append(query_idx)

            num_sentences = len(data_item["sentences"])

            for timestamp, exact_time, sentence, refined_sentence, ann_uid, query_idx in zipper:
                queries_idxs = sentence_query_map[refined_sentence]
                reference_query = query_idx == queries_idxs[0]
                if not reference_query and scope == "train":
                    continue
                elif reference_query:
                    start_times, end_times = [], []
                    for q_i in queries_idxs:
                        s_t = max(0.0, float(data_item["timestamps"][q_i][0]) / fps)
                        e_t = min(float(data_item["timestamps"][q_i][1]) / fps, duration)
                ...
                if self._predictor != "bert" and self._predictor != "RObertA_EgoVLPv2":
                ...
                record = {
                    ...
                    "refined_query": refined_sentence,
                    "num_text_queries": num_sentences,
                    "annotation_uid": ann_uid,
                    "query_idx": query_idx,
                    'queries_idxs': queries_idxs,
                    'reference_query': reference_query,
                    'num_id_queries': len(queries_idxs),
                }
                if (
...
def dataset_gen(
    ...
    def worker(
        ...
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
            ...
            start_end_times = np.array([record["s_time"], record["e_time"]])
            if len(start_end_times.shape) == 1:
                start_end_times = start_end_times[None, :]

            start_end_inds = batched_time_to_index(start_end_times, vfeat_lens[vid], record["duration"])
            s_inds, e_inds = start_end_inds[:, 0], start_end_inds[:, 1]
...
def dataset_gen_bert(
    ...
    # Worker method for multiprocessing.
    def worker(
        worker_data, vfeat_lens, original_vfeat_lens, tokenizer, max_pos_len, scope, worker_id, output_q
    ):
        worker_dataset = list()
        description = f"process {scope} data [{worker_id}]"
        # Cache candidates
        ...
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
            ...
            if record["reference_query"]:
                start_end_times = np.asarray([record["s_times"], record["e_times"]])
            else:
                start_end_times = np.asarray([[record["s_time"]], [record["e_time"]]])
            ...
            start_end_inds = batched_time_to_index(start_end_times, vfeat_lens[vid], record["duration"])
            # if record["reference_query"]:
                # print("Start and end times", record["s_times"], record["e_times"])
                # print("Current s_indxs, e_indxs", start_end_inds[0], start_end_inds[1])
            s_indxs, e_indxs = start_end_inds[0], start_end_inds[1]
            ...
            word_ids = tokenizer(record["refined_query"])
            result = {
                "sample_id": record["sample_id"],
                "vid": record["vid"],
                "s_time": record["s_time"],
                ...
                "num_text_queries": record["num_text_queries"],
                "s_indxs": s_indxs,
                "e_indxs": e_indxs,
                "v_len": vfeat_lens[vid],
                "v_original_len": original_vfeat_lens[vid],
                "w_ids": word_ids,
                "annotation_uid": record["annotation_uid"],
                "query_idx": record["query_idx"],
                "queries_idxs": record["queries_idxs"],
                "reference_query": record["reference_query"],
                "num_id_queries": record["num_id_queries"],
            }
    ...
    for worker_id in range(num_workers):
        allotment = data[worker_id * chunk_size : (worker_id + 1) * chunk_size]
        inputs = (allotment, vfeat_lens, original_vfeat_lens, tokenizer, max_pos_len, scope)
        inputs += (worker_id, output_q)
        process = multiprocessing.Process(target=worker, args=inputs)
        jobs.append(process)
        process.start()
...
def gen_or_load_dataset(configs):
    ...
    if configs.suffix is None:
        save_path = os.path.join(
            configs.save_dir,
            "_".join(
                [configs.task, configs.fv, str(configs.max_pos_len), configs.predictor, "grouped", "final"]
            )
            + ".pkl",
        )
    else:
    ...
    if os.path.exists(save_path):
        print("Loading dataset from pickle file:", save_path)
    ...
    original_vfeat_lens = vfeat_lens.copy()
    ...
    if configs.predictor == "bert" or configs.predictor == "RObertA_EgoVLPv2":
        from transformers import BertTokenizer, AutoTokenizer
        if configs.predictor == "RObertA_EgoVLPv2":
            print("Using BERT tokenizer for EgoVLPv2 features")
            tokenizer = AutoTokenizer.from_pretrained("roberta-base", TOKENIZERS_PARALLELISM = False)
        else:
            print("Using BERT tokenizer for VSLNet architecture")
        ...
        print("Using GloVe embeddings (Posible failure point)")
