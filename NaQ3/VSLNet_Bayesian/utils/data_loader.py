...
from utils.data_util import pad_seq, pad_char_seq, pad_video_seq, time_to_index, new_time_to_index
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.features_sampling_idxs = features_sampling_idxs
    ...
    def __getitem__(self, index):
        ...
        sampling_idx = self.features_sampling_idxs[record["vid"]]

        s_indxs, e_indxs = record["s_indxs"], record["e_indxs"]
        word_ids = record["w_ids"]
        char_ids = record.get("c_ids", None)
        return record, video_feature, sampling_idx, word_ids, char_ids, s_indxs, e_indxs
    ...
    def expand_narration_window(self, query, s, e, maxlen):
        if query.startswith("#") and self.nar_rand_exp_factor > 0:
            print("Expanding narration window from query:", query, "will fail because is not implemented yet")
            c = (s + e) / 2.0
            w = e - s + 1
    ...
    def expand_narration_window_from_idx(self, query, idx, maxlen):
        if query.startswith("#") and self.nar_rand_exp_factor > 0:
            print("Expanding narration window from query:", query, "will fail because is not implemented yet")
            s = idx[0]
            e = idx[1]
...
def train_collate_fn(data):
    records, video_features, features_sampling_idxs, word_ids, char_ids, s_inds, e_inds = zip(*data)
    ...
    s_labels = [torch.tensor(arr, dtype=torch.int64) for arr in s_inds]
    e_labels = [torch.tensor(arr, dtype=torch.int64) for arr in e_inds]
    h_labels = torch.tensor(h_labels, dtype=torch.int64)
    vfeat_idxs = features_sampling_idxs
    return records, vfeats, vfeat_lens, vfeat_idxs, word_ids, char_ids, s_labels, e_labels, h_labels


def test_collate_fn(data):
    records, video_features, features_sampling_idxs, word_ids, char_ids, *_ = zip(*data)
    # If BERT is used, pad individual components of the dictionary.
    if not isinstance(word_ids[0], list):
        pad_input_ids, _ = pad_seq([ii["input_ids"] for ii in word_ids])
        pad_attention_mask, _ = pad_seq([ii["attention_mask"] for ii in word_ids])
    ...
    vfeat_lens = torch.tensor(vfeat_lens, dtype=torch.int64)
    vfeat_idxs = features_sampling_idxs
    return records, vfeats, vfeat_lens, vfeat_idxs, word_ids, char_ids


def get_train_loader(dataset, video_features, features_sampling_idxs, configs, is_distributed=False):
    train_set = Dataset(
        dataset=dataset,
        video_features=video_features,
        features_sampling_idxs=features_sampling_idxs,
...
def get_test_loader(dataset, video_features, features_sampling_idxs, configs, is_distributed=False):
    test_set = Dataset(dataset=dataset, video_features=video_features, features_sampling_idxs=features_sampling_idxs)
    test_sampler = None
    if is_distributed:
