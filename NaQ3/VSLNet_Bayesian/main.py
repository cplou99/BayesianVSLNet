...
def main(configs, parser):
    ...
    print("Local rank: ", local_rank, "World rank: ", world_rank, "World size: ", world_size)
    ...
    if is_distributed:
        ...
        if world_rank == 0:
            ...
            print("**********************************************")
            visual_features, idxs_features = load_video_features(
                os.path.join("data", "features", configs.fv), configs.max_pos_len, configs.load_omnivore, configs.sampling
            )
        torch.distributed.barrier()
    ...
    if configs.video_agnostic:
        print("Randomizing video features")
        visual_features = {
    ...
    train_loader, train_sampler = get_train_loader(
        ...
        features_sampling_idxs = idxs_features,
        configs=configs,
    ...
    # create model dir
    model_name = configs.fv
    if configs.load_omnivore: model_name += "_Omnivore"
    if configs.pretrained_model_path != "": model_name += "_NaQpretrained"
    if configs.gradual_mlp: predictor=configs.predictor + "_gradualMLP"
    else: predictor=configs.predictor
    ...
    if configs.mode.lower() == "train":
        if configs.video_feature_dim == 2304: name1 = 'VLSNet_EgoVLPOmnivore_NO_text_video_heads'
        elif configs.video_feature_dim == 768: name1 = 'VLSNet_EgoVLP_NO_text_video_heads'
        elif configs.video_feature_dim == 1536: name1 = 'VLSNet_Omnivore'
        elif configs.video_feature_dim == 4096: name1 = 'VLSNet_EgoVLP_with_text_video_heads'
        elif configs.video_feature_dim == 5632: name1 = 'VLSNet_EgoVLPOmnivore_with_text_video_heads'

        exp_name = f'{name1}_Sampling{configs.max_pos_len}_Dim{configs.dim}_gradualMLP{configs.gradual_mlp}_sampling{configs.sampling}_NewProbsPredOurNNWeightedLoss'
        if configs.pretrained_model_path != "":
            exp_name += "_NaQpretrained"
        wandb.init(project="StepGroundingChallenge", name=exp_name, config=configs)
        ...
        if configs.pretrained_model_path != "":
            ...
            if configs.predictor == "bert":
                ckpt_model.pop('embedding_net.embedder.embeddings.position_ids')
            if configs.max_pos_len != 128:
                ckpt_model.pop('predictor.encoder.pos_embedding.position_embeddings.weight')
                ckpt_model.pop('feature_encoder.pos_embedding.position_embeddings.weight')
        ...
        for epoch in range(configs.epochs):
            ...
            for data in iterator:
                ...
                s_labels, e_labels, h_labels = (
                    [s_label.to(device) for s_label in s_labels],
                    [e_label.to(device) for e_label in e_labels],
                    h_labels.to(device),
                )
                ...
                if (
                    ...
                    train_dict = {"Train/Loss/Total": total_loss.detach().cpu(),  "Train/LR": optimizer.param_groups[0]["lr"]}
                ...
                if world_rank == 0 and (
                    ...
                    original_keys = list(score_dict.keys())
                    for key in original_keys:
                        new_key = "Val/" + key
                        score_dict[new_key] = score_dict.pop(key)
        ...
        # get last checkpoint file
        filename = get_last_checkpoint(model_dir, suffix="t7")
        print("Filename used for testing", filename)
        model.load_state_dict(torch.load(filename))
        model.eval()
